...
import com.ecdial.activity.AllNewArrivalsActivity;
...
import com.ecdial.adapter.NewArrivalHomeAdapter;
...
public class HomeFrag extends Fragment {
    ...
    View view;
    Context context;
    RecyclerView.LayoutManager layoutManager, layoutManagerUpcoming, layoutManagerOffer, layoutManagerNewArrivals;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding.ivAllArrival.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(getActivity(), AllNewArrivalsActivity.class));
                Animatoo.animateSwipeRight(getActivity());
        ...
        AndroidNetworking.post(APIDATA.BASE_URL)
                ...
                .getAsObject(AllBrandsResponce.class, new ParsedRequestListener<AllBrandsResponce>() {
                    ...
                    public void onResponse(AllBrandsResponce response) {
                        ...
                        Log.e("fhfghfgh", "onResponse: " + response.getData().get(0).getBrandName());
                        if (!response.getData().isEmpty()) {
                ...
                .getAsObject(NewArrivalsModel.class, new ParsedRequestListener<NewArrivalsModel>() {
                    ...
                    public void onResponse(NewArrivalsModel response) {
                        ...
                        if (!response.getData().isEmpty()) {
                            ...
                            ArrayList<NewArrivalsModel.Datum> newArrivalList = new ArrayList<>();

                            if (response.getData().size() > 4) {

                                for (int i = 0; i < 4; i++) {
                                    newArrivalList.add(response.getData().get(i));
                                }

                                binding.rvNewArrival.setHasFixedSize(true);
                                layoutManagerNewArrivals = new GridLayoutManager(getActivity(), 2, RecyclerView.VERTICAL, false);
                                binding.rvNewArrival.setLayoutManager(layoutManagerNewArrivals);
                                NewArrivalHomeAdapter adapter = new NewArrivalHomeAdapter(getActivity(), newArrivalList);
                                binding.rvNewArrival.setAdapter(adapter);
